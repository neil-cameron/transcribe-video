...
# )
diar = Pipeline.from_pretrained("pyannote_models/config.yaml")
...
for file_counter, file_path_item in enumerate(full_file_path_list):
    ...
    if args.number_speakers > 1:  # We need diarization
        ...
        diarization = diar(str(wav_audio_file_path))
        ...
        for speech_turn, _, speaker_label in diarization.itertracks(yield_label=True):
            ...
            if first_speaker == -1:
                first_speaker = speaker_label
                current_speaker = speaker_label
                current_speaker_start = speech_turn.start
            if speaker_label != current_speaker:
                dict_to_add = {
                    "start": current_speaker_start,
                    "end": speech_turn.start,
